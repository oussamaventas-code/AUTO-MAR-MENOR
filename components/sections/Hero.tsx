  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 -z-10">
        ...
        <div className="hero-overlay absolute inset-0 bg-ink-950/40 opacity-0" />
        <div className="absolute inset-0 bg-bosch-glow opacity-30 mix-blend-screen" />
      ...
      <div className="container-x absolute inset-x-0 top-[110px] flex items-center justify-between font-mono text-[10px] uppercase tracking-[0.3em] text-bone-300/80 md:text-[11px]">
        <span className="flex items-center gap-2">
          <span className="inline-block h-px w-6 bg-bosch" />
          37.99°N · 1.13°W
        </span>
        <span className="hidden md:flex items-center gap-2">
          Bosch Car Service · Oficial
          <span className="inline-block h-px w-6 bg-bosch" />
        </span>
      </div>
      ...
      <div className="container-x relative flex min-h-[100svh] flex-col justify-end pb-24 pt-44 md:justify-center md:pb-32 md:pt-32">
        <div className="hero-content max-w-[1100px]">
          ...
          >
            <span className="block">Tecnología BOSCH</span>
            <span className="shine-text block">precisión alemana.</span>
          </motion.h1>
          ...
          >
            Diagnosis electrónica, mecánica integral y mantenimiento oficial Bosch para
            que tu coche rinda como el primer día. En Murcia, desde hace más de 20 años.
          </motion.p>
          ...
          {STATS.map((s) => (
            <div key={s.label} className="bg-ink-950/40 px-6 py-6 backdrop-blur">
              <div className="flex items-baseline gap-1 font-display text-3xl tracking-normal text-white md:text-5xl">
                <Counter to={s.value} />
                <span className="text-bosch">{s.suffix}</span>
              </div>
      ...
      >
        <div className="flex flex-col items-center gap-2">
          <span>Desliza</span>
          <span className="block h-8 w-px animate-pulse-slow bg-bosch" />
        </div>
      </motion.div>
    </section>
  );
