...
export function CtaFinal() {
  ...
  return (
    ...
    >
      <div
        aria-hidden
        className="cta-glow pointer-events-none absolute left-1/2 top-1/2 h-[700px] w-[700px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-bosch-glow opacity-20 blur-3xl"
      />
      <div
        aria-hidden
      ...
      <div className="container-x relative text-center">
        ...
        >
          <span className="shine-text">servicio de precisión.</span>
        </SplitReveal>

        <p className="mx-auto mt-10 max-w-xl text-bone-300 md:text-lg">
          Reserva tu cita hoy y recibe un diagnóstico Bosch profesional. Sin compromiso,
          sin sorpresas, sin tiempos de espera.
        </p>
        ...
        <div className="mt-16 flex flex-wrap items-center justify-center gap-x-10 gap-y-4 font-mono text-[11px] uppercase tracking-[0.25em] text-bone-300">
          <span className="flex items-center gap-2">
            <Icon name="pin" className="h-4 w-4 text-bosch" />
            {BRAND.address.street}, {BRAND.address.locality}
          </span>
          <span className="flex items-center gap-2">
            <Icon name="clock" className="h-4 w-4 text-bosch" />
            L-V 08:30 — 14:00 · 16:00 — 19:00
          </span>
          <span className="flex items-center gap-2">
            <Icon name="shield" className="h-4 w-4 text-bosch" />
            Garantía Bosch
          </span>
        </div>
        <ContactForm />
...
function ContactForm() {
  ...
  if (sent) {
    return (
      <div className="mx-auto mt-16 max-w-lg rounded-2xl border border-bosch/20 bg-bosch/5 p-8 text-center backdrop-blur">
        <div className="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-full bg-bosch/10">
          <Icon name="check" className="h-7 w-7 text-bosch" />
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-4 pt-4 md:grid-cols-2">
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder:text-bone-300/30 outline-none transition-colors focus:border-bosch"
          />
        </div>
        <div className="space-y-1">
          <label className="pl-1 font-mono text-[10px] uppercase tracking-wider text-bone-300">
        ...
        <div className="space-y-1">
          ...
          <select
            ...
            required
            defaultValue=""
            className="w-full appearance-none rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white outline-none transition-colors focus:border-bosch [&>option]:text-ink-950"
          >
            <option value="" disabled hidden>Selecciona un servicio</option>
            <option value="Revisión oficial / Mantenimiento">Revisión oficial / Mantenimiento</option>
            <option value="Cambio de aceite y filtros">Cambio de aceite y filtros</option>
            <option value="Neumáticos">Neumáticos</option>
            <option value="Frenos (Pastillas / Discos)">Frenos (Pastillas / Discos)</option>
            <option value="Diagnosis electrónica">Diagnosis electrónica</option>
        ...
        <div className="space-y-1">
          ...
          <div className="relative">
            <input
              ...
              required
              min={today}
              className="w-full appearance-none rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white outline-none transition-colors focus:border-bosch [&::-webkit-calendar-picker-indicator]:invert"
            />
          </div>
        </div>
        ...
        <div className="space-y-1 md:col-span-2">
          ...
          <textarea
            ...
            className="w-full resize-none rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder:text-bone-300/30 outline-none transition-colors focus:border-bosch"
          />
        </div>
      </div>
      ...
      <p className="text-center text-[10px] text-bone-300/80">
        ...
        <a href="/politica-privacidad" className="text-bosch hover:underline">
