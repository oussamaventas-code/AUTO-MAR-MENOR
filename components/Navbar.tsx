          ...
          >
            ...
            <nav className="hidden items-center gap-7 lg:flex">
              {LINKS.map((l) => (
                ...
                >
                  {l.label}
                  <span className="pointer-events-none absolute -bottom-1 left-0 h-px w-full origin-left scale-x-0 bg-bosch-glow transition-transform duration-500 group-hover:scale-x-100" />
                </a>
              ))}
            </nav>
            ...
            <div className="flex items-center gap-2">
              <a
                ...
                className="hidden items-center gap-2 rounded-full border border-white/10 px-4 py-2 font-mono text-[11px] uppercase tracking-[0.2em] text-bone-200 transition-colors hover:border-bosch/50 hover:text-white md:inline-flex"
              >
                <span className="relative flex h-2 w-2">
                  <span className="absolute inset-0 animate-ping rounded-full bg-bosch/60" />
                  <span className="relative inline-flex h-2 w-2 rounded-full bg-bosch" />
                </span>
                {BRAND.phoneDisplay}
              </a>
...
function Logo() {
  return (
    <span className="relative grid h-10 w-10 place-items-center overflow-hidden rounded-full border border-white/10 bg-ink-900">
      <span className="absolute inset-0 bg-bosch-glow opacity-0 blur-xl transition-opacity duration-500 group-hover:opacity-100" />
      <span className="relative font-display text-lg font-bold tracking-normal text-white">
        AM
      </span>
      <span className="absolute -bottom-1 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full bg-bosch" />
    </span>
  );
}
